...
async function activate(context: any) {
    ...
    const originalProvideHover =
        dartExtension.exports._privateApi.analyzer.client._clientOptions.middleware.provideHover;

    // Override the provideHover method.
    dartExtension.exports._privateApi.analyzer.client._clientOptions.middleware.provideHover =
        async function (document: any, position: any, token: any, next: any) {
...
function formatParameters(code) {
    const typeRegex =
        /((?:required\s+)?([\w<>?]+(\s+Function\([^)]*\))?\??)\s+(\w+)(\s+=\s+\w+)?)/g;
    return code.replace(typeRegex, (match, _, type, __, name, defaultValue) => {
